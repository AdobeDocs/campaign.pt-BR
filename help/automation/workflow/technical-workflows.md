...
Os fluxos de trabalho detalhados nesta seção são instalados com os diferentes pacotes integrados do Adobe Campaign. Esses pacotes e os fluxos de trabalho técnicos relacionados dependem do contrato de licença. Os pacotes incorporados são detalhados em .
...
|------|--------|-----------|
| **Limpeza de alias** (aliasCleansing) | Instalado por padrão | Esse fluxo de trabalho padroniza os valores da lista discriminada. É acionado todos os dias às 3h por padrão. |
...
| **Tarefas do Campaign** (operationMgt) | Instalado por padrão | Esse fluxo de trabalho gerencia as tarefas para campanhas de marketing (inicia o direcionamento, faz a extração de arquivos etc.). Ele também cria workflows relacionados a campanhas recorrentes e periódicas. |
...
| **Trabalhos em deliveries em campanhas** (deliveryMgt) | Instalado por padrão | Esse fluxo de trabalho aciona os deliveries aprovados e inicia o pós-processamento no provedor de serviços para um delivery externo. Também envia notificações e lembretes de aprovação. |
| **Trabalhos em prestadores de serviços** (supplierMgt) | Instalado por padrão | Esse fluxo de trabalho inicia processos do provedor de serviços (email para o roteador e o pós-processamento) após a aprovação de deliveries. |
...
| **Gerenciamento de recusa de NMAC** (mobileAppOptOutMgt) | Canal de aplicativo móvel (Push) | Esse workflow atualiza a notificação de unsubscriptions em dispositivos móveis. É acionado a cada 6 horas entre 1:00 AM e meia-noite. Para obter mais detalhes, consulte . |
...
| **Limpeza de fluxos de trabalho pausados** (cleanupPausedWorkflows) | Instalado por padrão | Esse workflow analisa workflows pausados que têm a severidade definida como normal e emite avisos e notificações quando ficam pausados por muito tempo. Após um mês, os workflows técnicos pausados são interrompidos definitivamente. Por padrão, é acionado toda segunda-feira às 5h. Para obter mais informações, consulte[Tratamento de fluxos de trabalho pausados](monitor-workflow-execution.md#handling-of-paused-workflows). |
