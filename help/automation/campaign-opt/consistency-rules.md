...
Para definir a carga máxima, você precisa definir linhas de disponibilidade. Para fazer isso, duas opções estão disponíveis: você pode [criar uma ou mais linhas de disponibilidade](#add-availability-lines-one-by-one) ou criar intervalos de disponibilidade. A frequência desses períodos pode ser automatizada. [Saiba mais](#add-a-set-of-availability-lines).
